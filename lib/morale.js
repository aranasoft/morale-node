  ...
  var resCallback = function(res) {
    ...
    res.on('data', function(chunk) {
      if (res.statusCode == 200) {
	    ...
	    var data;
	    if (typeof chunk === 'string' && chunk != "") data = JSON.parse(chunk);
        var errMessage = (typeof data === 'object' && data.hasOwnProperty("error")) ? data.error : http.STATUS_CODES[res.statusCode];
