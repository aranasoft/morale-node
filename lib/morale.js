  ...
  var resCallback = function(res) {
    ...
    res.on('end', function() {
      if (res.statusCode == 200) {
	    ...
	    var jsonData;
	    if (typeof responseData === 'string' && responseData != "") jsonData = JSON.parse(responseData);
...
Morale.prototype.getTicket = function(projectId, ticketId, callback) {
  console.assert(typeof projectId === 'number' && projectId > 0, 'FAIL: INVALID PROJECTID');
  console.assert(typeof ticketId === 'number' && ticketId > 0, 'FAIL: INVALID TICKETID');
  console.assert(typeof callback === 'function', 'FAIL: INVALID CALLBACK');
  this._makeRequest('/api/v1/projects/' + projectId + '/tickets/' + ticketId, 'GET', callback);
  return this;
};
